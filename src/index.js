...
function activatePad(color) {
  // TODO: Write your code here.
  const pad = pads.find(pad => pad.color === color);
  //Add 'activated' class & play sound.
  pad.selector.classList.add("activated");
  pad.sound.play();
  //Remove 'activated' class from pad after 1/2 second.
  setTimeout(
    () => {
...
function playHumanTurn() {
    ...
    let remainingPresses = maxRoundCount - playerSequence.length;
...
function checkRound() {
  // TODO: Write your code here.
  const successMessage = "Congratulations! You won!";
  if (playerSequence.length === maxRoundCount) {
    resetGame(successMessage);
  } else {
    roundCount++;
    playerSequence = [];
    setText(statusSpan, `Nice! Keep going! Round ${roundCount}`);
...
function resetGame(text) {
  ...
  playerSequence = [];
  roundCount = 0;
  setText(text || "Game reset. Let's try again!");
